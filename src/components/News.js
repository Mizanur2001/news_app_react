...
export default class News extends Component {
    ...
    constructor(props) {
        ...
        document.title = `News Application - ${this.capitalize(this.props.category)}`
    }
        ...
        return (
            <>
                <div className='container'>
                    <h2 className='my-3'>{`${this.capitalize(this.props.category)} - Top Head Lines`}</h2>
                ...
                <div className="container flex-di-row d-flex justify-content-between my-5">
                    <button type="button" disabled={this.state.page <= 1} onClick={this.funcPrevious} className="btn btn-dark"> &larr; Previous</button>
                    <button type="button" disabled={this.state.page + 1 > Math.ceil(this.state.totalResults / this.props.pageSize)} onClick={this.funcNext} className="btn btn-dark">Next &rarr;</button>
                </div>
            </>
        )
    }
